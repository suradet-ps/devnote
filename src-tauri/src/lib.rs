...
fn init_logging(app_data_dir: &std::path::Path) {
  ...
  if let Err(e) = std::fs::create_dir_all(&log_dir) {
    eprintln!("devnote: failed to create log dir {:?}: {}", log_dir, e);
    return;
  }
  let log_file = log_dir.join("devnote.log");
  match std::fs::File::create(&log_file) {
    ...
    Err(e) => {
      eprintln!("devnote: failed to open log file {:?}: {}", log_file, e);
    },
  }
}
    ...
    .setup(|app| {
      ...
      // Capture file paths passed as command-line args (macOS "Open With" / drag-to-icon)
      {
        ...
        if !file_args.is_empty() {
          ...
          log::info!("[launch] no files from argv (macOS file-association uses Apple Event)");
        }
      }
